...
public class Action {
    ...
    public static boolean Autorization(Update update) {
        ...
        if ((idUser != 0) & (idChat != 0) & (idChat == idUser)) {
            ...
            LOG.Debug("Not private CHAT, empty message or other Error !!!", true);
        }
        CU.LogToDB();
        if (!CU.CloseDBConnection()) {
            LOG.Debug("DB Close error !!!", true);
            return false;
        }
        return true;
    }
        ...
        if (BOT.UIF.getAccess_menu() == 9) {
            switch (BOT.ModeArrOfStr[0]) {
                ...
                case "task":
                    ...
                    break;
                case "export": {
                    if (BOT.UIF.getAccess_vlan() == 9) {
                        switch (BOT.ModeArrOfStr[1]) {
                            case "GB":
                                BOT.SendMessage("Great Britain", "HTML", 0);
                                break;
                            case "PL":
                                BOT.SendMessage("Poland", "HTML", 0);
                                break;
                            case "IT":
                            case "ES":
                                BOT.SendMessage("Italy or Espnai", "HTML", 0);
                                break;
                            case "ELSE":
                            default: {
                          //      SendContact sndCon = new SendContact();
                          //      sndCon.setChatId("645111741");
                                BOT.SendMessage("https://t.me/mario_info_bot", "HTML", 0);
                                break;
                            }
                        }
                            }
                    BOT.SendMessage(Dictionary.Translate("Your choice : " + BOT.ModeArrOfStr[1], BOT.UIF.getLanguage()) + " This is Great !!!", "HTML", 1);
                    BOT.SendMessage(Dictionary.Translate("зв'яжіться з адміністратором, щоб надати вам доступ до сервісу пошуку! ", BOT.UIF.getLanguage()), "HTML", 1);
                    BOT.ModeArrOfStr[0] = "language";
                    ParseMenu(BOT);
                    break;
                }
                case "it": {
                            ...
                            switch (BOT.US.getModeTextArrOfStr(1)) {
                                case "sndmsg":
                                    if (BOT.DUMessage.SendSimplyMessageWithResponsOk(Long.valueOf(BOT.US.getModeTextArrOfStr(2)), BOT.MessageText)) {
                                        ...
                                        BOT.DUMessage.AdminMessage("private message to user " + Long.valueOf(BOT.US.getModeTextArrOfStr(2)) + " error !!!");
                                    }
